...
class Net {
 public:
  ...
  const vector<Blob<Dtype>*>& ForwardPrefilled_continue(int layerIdx, 
	  Dtype* loss = NULL);
  // Run forward using a set of bottom blobs, and return the result.
  const vector<Blob<Dtype>*>& Forward(const vector<Blob<Dtype>* > & bottom,
      Dtype* loss = NULL);
  const vector<Blob<Dtype>*>& Forward(const vector<Blob<Dtype>* >& bottom,
	  int layerIdx, Dtype* loss = NULL);
  const vector<Blob<Dtype>*>& Forward_continue(int layerIdx, 
	  Dtype* loss = NULL);
  void Forward_onelayer(int layerIdx, Dtype* loss = NULL);
  void Forward_onelayer(int layerIdx, vector<int>& bbox, Dtype* loss = NULL);
  ...
  void compute_bbox_for_layer(const vector<int>& bbox_in, vector<int>& bbox_out, const int layerIdx);
