...
class Layer {
 public:
  ...
  inline void spp_Forward_region(const vector<Blob<Dtype>*>& bottom,
  ...
  // compute coordinate. only implemented by conv layer and pooling layer. Add by byl
  virtual inline int kernel_h() const{
	  return -1;
  }
  virtual inline int kernel_w() const{
	  return -1;
  }
  virtual inline int padding_h() const{
	  return -1;
  }
  virtual inline int padding_w() const{
	  return -1;
  }
  virtual inline int stride_h() const {
	  return -1;
  }
  virtual inline int stride_w() const {
	  return -1;
  }
 ...
 protected:
  ...
  virtual void spp_Forward_region_cpu(const vector<Blob<Dtype>*>& bottom,
	  vector<Blob<Dtype>*>* top, const vector<int>& bbox) = 0;
...
template <typename Dtype>
inline void Layer<Dtype>::spp_Forward_region(const vector<Blob<Dtype>*>& bottom,
	  vector<Blob<Dtype>*>* top, const vector<int>& bbox){
  switch (Caffe::mode()) {
  case Caffe::CPU:
    spp_Forward_region_cpu(bottom, top, bbox);
  case Caffe::GPU:
    spp_Forward_region_gpu(bottom, top, bbox);
  default:
    LOG(FATAL) << "Unknown caffe mode.";
   // return;
	}
