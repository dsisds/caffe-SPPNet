...
template <typename Dtype>
const vector<Blob<Dtype>*>& Net<Dtype>::ForwardPrefilled_continue( int layerIdx, Dtype* loss) {
  if (loss != NULL) {
    *loss = Dtype(0.);
  }
  assert(layerIdx < layers_.size());
  for (int i = layerIdx; i < layers_.size(); ++i) {
    // LOG(ERROR) << "Forwarding " << layer_names_[i];
    Dtype layer_loss = layers_[i]->Forward(bottom_vecs_[i], &top_vecs_[i]);
    if (loss != NULL) {
      *loss += layer_loss;
...
template <typename Dtype>
const vector<Blob<Dtype>*>& Net<Dtype>::Forward_continue(
    int layerIdx, Dtype* loss) {
  // Copy bottom to internal bottom
  /*for (int i = 0; i < bottom.size(); ++i) {
    net_input_blobs_[i]->CopyFrom(*bottom[i]);
  }*/
  return ForwardPrefilled_continue(layerIdx, loss);
}

template <typename Dtype>
void Net<Dtype>::Forward_onelayer(
    int layerIdx, Dtype* loss) {
  if (loss != NULL) {
    *loss = Dtype(0.);
  }
  assert(layerIdx < layers_.size());
  Dtype layer_loss = layers_[layerIdx]->Forward(bottom_vecs_[layerIdx], &top_vecs_[layerIdx]);
  if (loss != NULL) {
     *loss += layer_loss;
  }
}
...
template <typename Dtype>
void Net<Dtype>::compute_bbox_for_layer(const vector<int>& bbox_in, vector<int>& bbox_out, const int layerIdx){
	for(int i=0;i<bbox_in.size();++i){
		bbox_out[i] = bbox_in[i];
	}
	//int W_h = 0, W_w = 0;
	//int P_h = 0, P_w = 0;
	int S_h = 1, S_w = 1;
	int height = bbox_out[2]-bbox_out[0]+1;
	int width = bbox_out[3]-bbox_out[1]+1;
	for(int i=0;i<=layerIdx;++i){
		if((layers_[i]->type() == LayerParameter_LayerType_CONVOLUTION) || 
			(layers_[i]->type() == LayerParameter_LayerType_POOLING)){
			// coordinate y
			const int kernel_h = layers_[i]->kernel_h();
			const int padding_h = layers_[i]->padding_h();
			const int stride_h = layers_[i]->stride_h();
			assert(kernel_h > -1);
			assert(padding_h > -1);
			assert(stride_h > -1);
			S_h *= stride_h;
			bbox_out[0] = (bbox_out[0] + padding_h) / stride_h;
			height = (height - kernel_h + 1) / stride_h;
			const int kernel_w = layers_[i]->kernel_w();
			const int padding_w = layers_[i]->padding_w();
			const int stride_w = layers_[i]->stride_w();
			assert(kernel_w > -1);
			assert(padding_w > -1);
			assert(stride_w > -1);
			S_w *= stride_w;
			bbox_out[1] = (bbox_out[1] + padding_w) / stride_w;
			width = (width - kernel_w + 1) / stride_w;
		}
	}
	bbox_out[2] = bbox_out[0] - 1 + height;
	bbox_out[3] = bbox_out[1] - 1 + width;
}

template <typename Dtype>
void Net<Dtype>::Forward_onelayer(int layerIdx, vector<int>& bbox, Dtype* loss) {
  if (loss != NULL) {
    *loss = Dtype(0.);
  }
  assert(layerIdx < layers_.size());
  vector<int> bbox_for_layer(4,0);
  compute_bbox_for_layer(bbox, bbox_for_layer, layerIdx - 1);
  layers_[layerIdx]->spp_Forward_region(bottom_vecs_[layerIdx], &top_vecs_[layerIdx], bbox_for_layer);
}
