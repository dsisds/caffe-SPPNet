...
//	LOG(ERROR) << "processing " << root_folder + lines[line_id].first;
    if (!ReadImageToDatum_crop(root_folder + lines[line_id].first,
        lines[line_id].second, resize_height, is_color, &datum)) {
      continue;
    }/*
	const string& img_data = datum.data();
	LOG(ERROR) << "channels:" << datum.channels() << " height:" << datum.height() << " width:" << datum.width();
	cv::Mat img_show(256, 256, CV_8UC3);
	for(int c=0;c<3;++c){
		for(int h=0;h<256;h++){
			for(int w=0;w<256;++w){
				int data_index = (c * 256 + h) * 256 + w;
				img_show.at<cv::Vec3b>(h,w)[c]= static_cast<uint8_t>(img_data[data_index]);
			}
		}
	}
	cv::imshow("im_show", img_show);
	cv::waitKey(0);*/
